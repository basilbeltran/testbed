        <div class="span2">
          <div class="well sidebar-nav">
            <ul class="nav nav-list">
            <li class="nav-header">datastores</li>
              <li><a href="#">Research</a></li>
              <li><a href="#">ci config</a></li>
              <li><a href="#">integration</a></li>
              <li><a href="#">functionality</a></li>
              <li><a href="#">CRUD & admin</a></li>
              <li><a href="#">mapping</a></li>
            <li class="nav-header">reporting</li>
              <li><a href="#">Research</a></li>              
              <li><a href="#">Evaluation</a></li>
              <li><a href="#">Testing</a></li>
              <li><a href="#">Reporting</a></li>
            <li class="nav-header">test engine</li>
              <li><a href="#">Research</a></li>
              <li><a href="#">Integraton</a></li>

                  
            
            </ul>
          </div><!--/.well -->
        </div><!--/span-->
        
        
        <div class="span10" id="HinFront">

          <div   class="row-fluid">
              
              
            <div class="span4">
              <h2>Caching</h2>
               <p>  APC can directly access local memory and cache parsed PHP code as well as user data.
                    Memcached only caches user data (SQL queries) but can leverage memory across multiple servers via TCP. 
                    Redis can ....
                 
              </p>
              <p><a class="btn" href="#">View details &raquo;</a></p>
            </div><!--/span-->
            <div class="span4">
              <h2>Dynamic Content</h2>
              <p>The amount of dynamic information you display in your pages will correlate 
                  directly to the server resources, memory, and processing cycles utilized, 
                  which affect your page load speeds. By caching your pages, since 
                  they are saved in their fully rendered state, you can achieve performance 
                  that nears that of static web pages. 
              </p>
              <p><a class="btn" href="#">View details &raquo;</a></p>
            </div><!--/span-->
            <div class="span4">
              <h2>Heading</h2>
              <p>Donec id elit non mi porta gravida at eget metus.  </p>
              <p><a class="btn" href="#">View details &raquo;</a></p>
            </div><!--/span-->
          </div><!--/row-->
          <div class="row-fluid">
            <div class="span4">
              <h2>Heading</h2>
              <p>Donec id elit non mi porta gravida at eget metus. </p>
              <p><a class="btn" href="#">View details &raquo;</a></p>
            </div><!--/span-->
            <div class="span4">
              <h2>Heading</h2>
              <p>Donec id elit non mi porta gravida at eget metus.  </p>
              <p><a class="btn" href="#">View details &raquo;</a></p>
            </div><!--/span-->
            <div class="span4">
              <h2>Heading</h2>
              <p>Donec id elit non mi porta gravida at eget metus.  </p>
              <p><a class="btn" href="#">View details &raquo;</a></p>
            </div><!--/span-->
          </div><!--/row-->
        </div> <!--/HinFront--> 


