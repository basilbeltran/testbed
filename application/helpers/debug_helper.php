...
function showGet() {
 $j = "";
 print(" show GET request variables <br>");

    foreach($_GET as $key => $j){
        print("$key=$j<br>");
    }
...
function _dump2(&$var, $level)
{
    ...
    elseif (is_array($var))
    {
        ...
        elseif ($level < $maxDepth || !$maxDepth)
        {
            ...
            $s .= "$space$space1$k => " . _dump2($v, $level + 1);
            }
            unset($var[$marker]);
    ...
    elseif (is_object($var))
    {
        ...
        elseif ($level < $maxDepth || !$maxDepth)
        {
            ...
            $s .= "$space$space1\"$k\"$m => " . _dump2($v, $level + 1);
            }
            array_pop($list);
            $s .= "$space}</code>";
        }
